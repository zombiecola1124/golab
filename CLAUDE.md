...
9. **보관(소프트 삭제)**: 품목 삭제 시 `status: 'DELETED'`로 소프트 삭제. DELETED 품목은 리스트/KPI에서 제외하되, '보관됨' 필터로 조회·복원 가능.

---

## 8-1. 정산 비즈니스 룰

> Phase 2(T9) 정산/증빙 구현 시 반드시 적용할 계산 규칙

1. **세금 우선 공제**: 정산 금액에서 세금 30%를 먼저 차감한다.
2. **리베이트 계산**: 세금 공제 후 남은 금액의 60%를 리베이트로 산정한다.
3. **계산 예시**:
   - 정산 원금: 1,000,000원
   - 세금 (30%): 300,000원
   - 세후 잔액: 700,000원
   - 리베이트 (60%): 420,000원
   - 순수익: 280,000원
4. **금액 표기**: 모든 정산 금액은 공급가(VAT 제외) 기준. VAT는 별도 행으로 표기.
